...
class SelectionApiTests(unittest.TestCase):
    ...
    def test_home_page_has_forms(self) -> None:
        ...
        self.assertIn("id=\"nl-drop\"", response.text)
        self.assertIn("id=\"nl-pdf\"", response.text)
        self.assertIn("attachDatasheetIfAny", Path(ROOT / "web" / "parse.js").read_text(encoding="utf-8"))
