...
- a background thread that re-probes on an interval and reloads when the ETag moves;
...
def _loop(stop: threading.Event, on_reload: Callable[[dict[str, Any]], None] | None) -> None:
    ...
    while not stop.is_set():
        ...
        if stop.is_set():
            return
        try:
            outcome = refresh_once(on_reload)
...
def start(on_reload: Callable[[dict[str, Any]], None] | None = None) -> threading.Event | None:
